package effects
{
    ...
    public class Effect25 extends Effect
    {   
        private const PIXELATE:int = 120;
        ...
        override protected function getProcessedImage(amount:Number):DisplayObject
        {
            ...
            for (var x:int = 0; x < resizedImage.width; x++)
            {
                for (var y:int = 0; y < resizedImage.height; y++)
                {
                    ...
                    m.translate(x*amount*(x%2 == 0 ? 1 : -1) + PIXEL_SIZE/2, y*amount*(y%2 == 0 ? 1 : -1) + PIXEL_SIZE/2);
                    ...
                    result.graphics.drawRect(x * PIXELATE, y * PIXELATE + pseudoRandom[x%pseudoRandom.length]*100, PIXEL_SIZE, PIXEL_SIZE);
