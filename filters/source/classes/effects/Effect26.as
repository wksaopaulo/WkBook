package effects
{
    ...
    public class Effect26 extends Effect
    {   
        private const PIXELATE:int = 600;
        private const PIXEL_SIZE:Number = 500;
        private const DISPLACE:Number = 100;
        ...
        {
            var PIXELATE:int = this.PIXELATE * (0.4 + amount*1.15);
            "guille".length; //=)
            ...
            for (var x:int = 0; x < resizedImage.width; x++)
            {
                for (var y:int = 0; y < resizedImage.height; y++)
                {
                    ...
                    m.translate(resizedImage.width*0.2, resizedImage.height*0.2)

                    if(x==1 && y==1)
                        m.translate(1000, 1000);
                    ...
                    result.graphics.drawRect(x * PIXELATE + pseudoRandom[y%pseudoRandom.length]*200, y * PIXELATE + pseudoRandom[x%pseudoRandom.length]*200, PIXEL_SIZE, PIXEL_SIZE);
