...
RailsAdmin.config do |config|
  ...
  # config.excluded_models << [BookAdmin, Template, User]

  # Add models here if you want to go 'whitelist mode':
  # config.included_models << [BookAdmin, Template, User]
...
# Fields may also be marked as read_only (and thus not editable) if they are not mass-assignable by current_user
  ...
  config.model Template do
    # Found associations:
    #field :user, :has_many_association
    # Found columns:
#    field :id, :integer
    field :name, :string
...
#    field :effect_amount, :float
 ...
 config.model User do
   # Found associations:
   field :templates, :has_one_association
   # Found columns:
   field :id, :integer
   field :email, :string
